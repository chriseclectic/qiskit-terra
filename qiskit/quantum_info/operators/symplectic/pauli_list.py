...
class PauliList(BasePauli):
    ...
    **Iteration**
    ...
    """
    def __init__(self, z, x=None, phase=None):
        """Initialize the PauliList.

        Args:
            z (array or str or list or Pauli or PauliList): input data or symplectic z vector.
            x (array): Optional, symplectic x vector.
            phase (array or int): input pauli phases.
        Raises:
        ...
        elif isinstance(z, PauliTable):
            # Conversion from legacy PauliTable
            base_z, base_x, base_phase = self._from_array(z.Z, x.X, phase)
        else:
            # Conversion as iterable of Paulis
            base_z, base_x, base_phase = self._from_paulis(z)

        # Set phase
        if phase is not None:
            base_phase = np.mod(
                np.sum(np.logical_and(base_x, base_z),
    ...
    def _from_paulis(data):
        ...
        base_z = np.zeros((num_paulis, num_qubits), dtype=np.bool)
        base_x = np.zeros((num_paulis, num_qubits), dtype=np.bool)
        base_phase = np.zeros(num_paulis, dtype=np.int)
    ...
    def __repr__(self):
        ...
        return self._truncate_list(show_class=True)

    def __str__(self):
        """Print representation."""
        return self._truncate_list(show_class=False)

    def _truncate_list(self, show_class=True):
        # pylint: disable=no-member
        threshold = None
        truncate = False
        prefix, suffix = '', ''
        if show_class:
            prefix, suffix = 'PauliList(', ')'
        if self.num_qubits > 100 or self._num_paulis * self.num_qubits > 2000:
            truncate = True
            threshold = 1
        if truncate and self._num_paulis > 6:
            front = self[:3]._truncate_labels(truncate)
            back = self[-3:]._truncate_labels(truncate)
            list_label = front + ['...'] + back
        else:
            list_label = self._truncate_labels(truncate)
        return '{}{}{}'.format(prefix, np.array2string(
            np.array(list_label), threshold=threshold, separator=', ',
            prefix=prefix), suffix)

    def _truncate_labels(self, truncate=False):
        if truncate and self.num_qubits > 4:
            return ['{}...{}'.format(back, front)
                    for front, back in zip(self[:, :2].label_iter(),
                                           self[:, -2:].label_iter())]
        return list(self.label_iter())
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, (int, np.int)):
            # Single Pauli
            return Pauli(BasePauli(self._z[index], self._x[index],
                                   self._phase[(index,)]))
